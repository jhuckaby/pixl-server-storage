...
Choose your `trans_dir` carefully.  For engines using the built-in commit path, this directory is used for rollback logs, and those logs are what make crash recovery possible.  Transaction record changes are kept in memory until commit time, but rollback logs are written to local disk and synced before the main storage engine is modified.  Engines using native commits do not write rollback logs for successful commits, but the transaction directory is still created for consistency and recovery bookkeeping.  In modern deployments, the best choice is usually a fast, reliable local path on the same machine as your Node.js process:

| Storage Setup | Recommendation |
|---------------|----------------|
| Postgres | Uses native PostgreSQL transactions during commit, so rollback logs are not used for successful commits.  You can still set `trans_dir` explicitly for consistency with other engines and future recovery behavior. |
| SQLite | Uses native SQLite transactions during commit, so rollback logs are not used for successful commits.  The default is usually fine, because SQLite exposes a `base_dir`, and transactions will default to a `_transactions` directory inside it. |
| Redis | Uses native Redis `MULTI` / `EXEC` transactions during commit, so rollback logs are not used for successful commits.  You can still set `trans_dir` explicitly for consistency with other engines and future recovery behavior. |
| RedisCluster | Uses the built-in rollback log system.  Set `trans_dir` explicitly to a fast local disk path. |
...
For engines that support native transactions, currently Postgres, Redis and SQLite, the engine commit path replaces the rollback log system entirely.  If the engine can safely reject or roll back a failed native commit, the error is returned to your application.  You should still call [abort()](API.md#abort), because it cleans up the outer transaction metadata and releases locks.  If the native engine detects an unsafe outcome, such as a connection failure while the commit decision is in progress or a failed database rollback, the storage system raises a fatal error immediately instead of returning a normal commit error.
...
The following engines currently support native commits:

- **Postgres** checks out one client from the `pg.Pool`, starts a PostgreSQL transaction with `BEGIN`, applies all final deletes and writes using set-based SQL statements, and then issues `COMMIT`.  The single checked-out client is important, because PostgreSQL transactions are connection-scoped.  The bulk SQL statements are important too, because they avoid one network round trip per record during commit.
- **Redis** queues all final writes and deletes into an ioredis `MULTI` / `EXEC` transaction.  Redis executes the queued commands atomically as a single transaction on the Redis server, and pixl-server-storage updates its own caches only after `EXEC` succeeds.
- **SQLite** uses the `better-sqlite3` native transaction wrapper.  The engine replays all final writes and deletes inside one SQLite transaction, so local multi-record commits are protected by SQLite's own transaction machinery.

The `Hybrid` engine can use native commits too, but only through its configured `docEngine`.  This works because pixl-server-storage transactions only include JSON records.  Binary keys are passed straight through outside the transaction system, so the `binaryEngine` is not part of the commit.  If the `docEngine` supports native commits, `Hybrid` exposes the native commit hook automatically.  If the `docEngine` does not support native commits, `Hybrid` uses the built-in rollback log path.

RedisCluster intentionally still uses the built-in rollback log path.  Redis Cluster transactions are only practical when all involved keys land in the same hash slot and on the same node.  pixl-server-storage transactions can touch many keys across a whole logical data structure, and those keys may be distributed across the cluster.  The rollback log path keeps RedisCluster behavior general and predictable without requiring special key-slot constraints.

If a native engine statement fails before the final commit decision, the engine returns an error to the outer transaction system if it can safely reject or roll back the native transaction.  Your application should still call [abort()](API.md#abort) on the transaction object, so pixl-server-storage can discard the in-memory branch and release its locks.

If a native engine reports an error while the final commit decision is in progress, or if a database rollback itself fails, the engine marks the error as fatal.  This is because the application may not be able to prove whether the database committed or rolled back, and there is no pixl-server-storage rollback log for this path.  The outer transaction system responds by raising a fatal storage error immediately.
...
For a native engine commit, rollback is handled differently.  If the engine can prove that its native transaction was rejected or rolled back, then pixl-server-storage only needs to discard its in-memory transaction branch and release locks.  If the engine cannot prove this, such as a connection failure during the final commit decision or a failed database rollback, the engine marks the error as fatal and pixl-server-storage shuts down immediately.
